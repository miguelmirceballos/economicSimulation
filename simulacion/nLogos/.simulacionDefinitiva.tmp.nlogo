...
to setup
  ...
  ;; Inicializamos lista-precios y demás variables nuevas
...
to go
    ...
    while [ necesidad > 0 ] [
      ...
      ifelse any? candidatos [
        ...
        let elegido ifelse-value any? cercanos
          [ min-one-of cercanos [ precio ] ]
          [ min-one-of candidatos [ distance myself ] ]
  ...
  ask producers [
    ...
    ;; Decay inmediato del inventario
    set inventario round (leftover * 0.7)
  ...
  ifelse length lista-precios > 1 [
    ...
    set volatilidad_precio sqrt ( mean squared-list )
  ] [
  ...
  ifelse length lista-precios > 4 [
    let anterior item (length lista-precios - 4) lista-precios
    let porcentaje_cambio ( precio_medio - anterior ) / max list anterior 1
    ifelse porcentaje_cambio > 0.1 [
      set subida_brusca_precio? true
    ] [
